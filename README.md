...
| **Protocol** | — | The director agent. Decomposes a Hypothesis into parallel tasks and assigns them to Pseudopods. Formerly *Director / Gru*. |
| **Pseudopod** | `/ˈsjuː.doʊ.pɒd/` | Worker agents — the extending "false feet" of a slime mold that explore the environment and bring results back. Formerly *Crew / Minion*. |
| **Assay** | `/ˈæs.eɪ/` | The reviewer agent. Performs quality analysis on Pseudopod output, including code review and risk evaluation. Formerly *Inspector*. |
...
| **Procedure** | — | A workflow definition executed in Temporal. The concrete steps to test a Hypothesis. Formerly *Blueprint*. |
| **PeerReview** | — | The merge-decision evaluator. Classifies PRs as `AUTO_MERGE`, `HUMAN_SUGGESTED`, or `HUMAN_REQUIRED`. Formerly *PRRiskEvaluator*. |
| **Experiment** | — | A unit of work. A Hypothesis being actively tested. Formerly *Mission*. |

### Environments & Memory

| Name | Pronunciation | Description |
|------|---------------|-------------|
| **Petri** | `/ˈpiː.tri/` | Preview / staging environment — an isolated dish for running experiments safely. Formerly *Launch Pad*. |
| **Publication** | — | Production environment — verified results released to the world. |
| **LabNote** | — | Global shared memory (S3 JSON). Knowledge accumulated across all experiments. Formerly *Ship's Log*. |
| **FieldNote** | — | Per-repository memory. Observations specific to one codebase. Formerly *Flight Log*. |
